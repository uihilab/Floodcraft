...
execute if score #state sf_state matches 0 if block 712 -46 469 #minecraft:buttons[powered=true] run function save_the_furniture:start
execute if score #state sf_state matches 2..3 if block 712 -46 469 #minecraft:buttons[powered=true] run function save_the_furniture:start
...
execute if score #state sf_state matches 2..3 if score #cooldown sf_timer matches ..0 run tellraw @a [{"text":"[Save the Furniture] ","color":"gold"},{"text":"Game reset! Press the button to play again.","color":"green"}]
