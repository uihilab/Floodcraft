...
execute if score #state sd_state matches 0 if block 611 -47 546 minecraft:dark_oak_button[powered=true] run function sandbag_defense:start
...
execute if score #state sd_state matches 2..3 if score #cooldown sd_timer matches ..0 run tellraw @a [{"text":"[Sandbag Defense] ","color":"gold"},{"text":"Game reset! Press the button to play again.","color":"green"}]
